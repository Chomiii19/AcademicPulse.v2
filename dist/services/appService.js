...
class AppService {
    ...
    async validateIdService(req) {
        ...
        if (!student.isEnrolled)
            throw new AppError("Student is not yet enrolled for this school year.", 400);
        if (await Validated.findOne({ studentNumber: student.studentNumber }))
            throw new AppError("Student is already validated for this school year.", 409);
    ...
    async entranceLogService(req) {
        ...
        if (!student)
            throw new AppError("Student is not validated!", 404);
    ...
    async validatedStatsService(req) {
    ...
    formatData = function (data, type) {
        ...
        if (type === "hour") {
            entryTimes = data.entryTimes.map((entry) => entry.hour !== undefined ? entry.hour % 24 : 0);
            exitTimes = data.exitTimes.map((exit) => exit.hour !== undefined ? exit.hour % 24 : 0);
        }
        else if (type === "day") {
            entryTimes = data.entryTimes.map((entry) => entry.day !== undefined ? entry.day : 0);
            exitTimes = data.exitTimes.map((exit) => exit.day !== undefined ? exit.day : 0);
        }
        else {
            entryTimes = data.entryTimes.map((entry) => entry.month !== undefined ? entry.month : 0);
            exitTimes = data.exitTimes.map((exit) => exit.month !== undefined ? exit.month : 0);
        }
    ...
    async schoolLogsService(req) {
        const { year, month, day } = req.query;
        const daysInMonth = new Date(Number(year), Number(month), 0).getDate();
        let groupbyEntry = {}, groupbyExit = {}, filterEntry = {}, filterEnd = {}, fieldNameEntry = {}, fieldNameExit = {}, project = {}, dataLog = {
            entryLogs: [],
            exitLogs: [],
        };
        let addField, avgExit = { $toLong: "$exitTime" }, avgEntry = { $toLong: "$entryTime" };
        ...
        if (year && month && day) {
            const { start, end } = this.utcDate(`${year}-${month}-${day}`, `${year}-${month}-${day}`);
            filterEntry = {
                entryTime: {
                    $gte: start,
                    $lte: end,
            ...
            const { start, end } = this.utcDate(`${year}-${month}-01`, `${year}-${month}-${daysInMonth}`);
            filterEntry = {
                entryTime: {
                    $gte: start,
                    $lte: end,
        ...
        if (year && month && day)
            dataLog = this.formatData(data[0], "hour");
        else if (year && month)
            dataLog = this.formatData(data[0], "day");
        else if (year)
            dataLog = this.formatData(data[0], "month");
        return dataLog;
    }
    async studentLogStatsService(req) {
        const { date, studentNumber } = req.body;
        let fillDateStart = date, fillDateEnd = date;
        if (!date) {
    ...
    async validatedStatService(req) {
        const data = await Validated.aggregate([
            { $group: { _id: null, count: { $sum: 1 } } },
            { $project: { _id: 0, count: 1 } },
        ]);
    ...
    async studentsInSchoolService(req) {
        const data = await StudentLog.aggregate([
            { $match: { inSchool: true } },
            { $group: { _id: null, count: { $sum: 1 } } },
            { $project: { _id: 0 } },
