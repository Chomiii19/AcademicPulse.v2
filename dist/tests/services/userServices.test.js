...
describe("UserService", () => {
    it("should throw error if token is not provided in checkToken", async () => {
        await expect(UserService.checkToken("")).rejects.toThrowError(new AppError("You are not logged in!", 401));
    });
    it("should successfully decode token in checkToken", async () => {
        ...
        verifyToken.mockResolvedValueOnce(decoded); // Mock the return value of verifyToken
    ...
    it("should throw error if user is not found in findById", async () => {
        ...
        await expect(UserService.findById(mockUserId)).rejects.toThrowError(new AppError("User does not exist.", 404));
    });
});
